public class Token {
    ...
    private static final String[] reserved = new String[KEYWORDS];  // Keyword에 대한 String
    private static Token[] token = new Token[KEYWORDS];             // Keyword에 대한 객체
    ...
    private Token (TokenType t, String v) {
        type = t;
        value = v;
        if (t.compareTo(TokenType.Eof) < 0) { // Eof보다 작은 keyword들
            int ti = t.ordinal();   // 몇번째 token인가
            reserved[ti] = v;
            token[ti] = this;
        }
    }
    ...
    public static Token keyword  ( String name ) {
        ...
        if (ch >= 'A' && ch <= 'Z') return mkIdentTok(name);    // keyword는 소문자로 시작
        for (int i = 0; i < KEYWORDS; i++)
           if (name.equals(reserved[i]))  return token[i];      // keyword 객체 반환
        return mkIdentTok(name);                                // Keyword가 아니면 식별자로 반환
    } // keyword
    ...
    public String toString ( ) {
        if (type.compareTo(TokenType.Identifier) < 0) return value; // identifier부터는 value도 출력
        return type + "\t" + value;
    } // toString
