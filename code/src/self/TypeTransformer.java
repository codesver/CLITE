    ...
    public static Expression T(Expression e, TypeMap tm) {
        ...
        if (e instanceof Unary) {
            ...
            if (type == Type.BOOL && u.op.NotOp())
                return new Unary(Operator.boolMap(u.op.val), t0);
            else if (type == Type.FLOAT && u.op.NegateOp()) 
                return new Unary(Operator.floatMap(u.op.val), t0);
            else if (type == Type.INT && u.op.NegateOp())
                return new Unary(Operator.intMap(u.op.val), t0);
            else if (type == Type.INT && u.op.floatOp())
                return new Unary(Operator.intMap(u.op.val), t0);
            else if (type == Type.INT && u.op.charOp())
                return new Unary(Operator.intMap(u.op.val), t0);
            else if (type == Type.FLOAT && u.op.intOp())
                return new Unary(Operator.floatMap(u.op.val), t0);
            else if (type == Type.CHAR && u.op.intOp())
